...
//#include "Runtime/RenderCore/Public/RenderCore.h"   //wei le   ftimer   er  sheng
#include <windows.h>
#define WIN32 0
...
void FSlateD3DRenderer::DrawWindows( FSlateDrawBuffer& InWindowDrawBuffer )
{
	...
	for( int32 ListIndex = 0; ListIndex < WindowElementLists.Num(); ++ListIndex )
	{
		...
		if ( ElementList.GetWindow().IsValid() )
		{
			...
			g_Count++;
			//	来源于GetTimeInMiliseconds()---------------------------------------------------------------------------------time   1---
			double time_liuBegin = 99.0;
			double time_liuEnd = 99.0;
			...
			if (s_use_qpc)
			{
				...
				time_liuBegin = ((double) now.QuadPart ) / (double)s_frequency.QuadPart;//这是毫秒级别的精度//不准 多了 2
								 //1000LL *     //不加1000就是秒级别了								
			}
			//else
			//{
			//	time_liuBegin = GetTickCount();
			//}
		...
		//	Sleep(1000);
			...
			Viewport->D3DSwapChain->Present(bUseVSync ? 1 : 0, 0);//bUseVSync ? 1 : 0
			
			
			QueryPerformanceCounter(&nEndTime);
			double Experience_time = ((double)( nEndTime.QuadPart - nBeginTime.QuadPart)  )/(double)s_frequency.QuadPart ;//这是微秒精度的 时间段的时间,最后这里是最准确的了



			
		//	来源于GetTimeInMiliseconds()--------------------------------time---2---------------------
		...
		//		FTimer timeTemp;///////////////////////////////////////////////////////////////////////////
				if (s_use_qpc)
				{
					LARGE_INTEGER now;
					QueryPerformanceCounter(&now);
			//		float timeByFtimer = timeTemp.GetTickCount();//////////////////////////////////////
					time_liuEnd =((double)now.QuadPart) / (double)s_frequency.QuadPart;//这是毫秒级别的精度
					//1000LL *     //不加1000就是秒级别了
				}
				//else
				//{
		...
		//		float timeByFtimer=	timeTemp.GetTickCount();//////////////////////////////////////////////
				//	time_liuEnd = GetTickCount();
				//}
			//	 Experience_time = time_liuEnd - time_liuBegin;//经历的时间
				UE_LOG(LogStandaloneRenderer, Error, TEXT("计数:%d 呈现前时刻: %f秒  Experience_time: %f秒  呈现结束后时刻: %f秒"), g_Count, time_liuBegin, Experience_time, time_liuEnd);//---------------------------------------------time 1----------------------------------
		//		UE_LOG(LogStandaloneRenderer, Log, TEXT("计数:%d Experience_time: %f"), g_Count, Experience_time);//---Experience_time
		//	UE_LOG(LogStandaloneRenderer, Log, TEXT("计数:%d 呈现结束后时刻time: %f"), g_Count, time_liuEnd);//--------------------------------------------time---2----------------------------------
